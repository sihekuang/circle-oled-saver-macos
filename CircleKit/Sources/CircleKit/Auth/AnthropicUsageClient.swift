    ...
    public static func decode(_ data: Data) throws -> AnthropicUsage {
        ...
        decoder.dateDecodingStrategy = .custom { d in
            ...
            throw DecodingError.dataCorruptedError(
                in: container,
                debugDescription: "Unrecognized resets_at format: \(s)"
            )
        }
...
/// Calls the same undocumented `/api/oauth/usage` endpoint that Claude Code
/// and the Claude Desktop app use to render their quota status. Authenticated
/// with the user-provided OAuth access token (paste-into-settings flow), not
/// extracted from the system keychain.
public final class AnthropicUsageClient {
    ...
    public init(
        ...
        tokenProvider: @escaping () -> String? = {
            KeychainStore.get(service: KeychainStore.claudeOAuthService)
        }
    ) {
